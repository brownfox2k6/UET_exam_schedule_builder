...
Updated for Room + ProblemData-based AntColony constructor with ExamSection.
No proctor model.
"""
...
def sample_data():
    ...
    for i in range(num_exams):
        ...
        group_id = i // 10
        # Tạo danh sách sinh viên cho lớp học phần
        students = [f"{24000000 + group_id * 100 + idx:08d}" for idx in range(15)]

        # Bọc danh sách sinh viên vào ExamSection theo API mới
        section = solver.ExamSection(f"{code}_SEC1", students)
        ...
        # Khởi tạo Exam với list chứa các ExamSection
        exams.append(
            solver.Exam(code, credits, [section], feasible_slots, room_indices)
        )

    return exams, timestamps, rooms, hp


def test_exam_and_section_initialization_and_properties():
    # 1. Test ExamSection
    section1 = solver.ExamSection("CS101_1", ["24000001", "24000002"])
    section2 = solver.ExamSection("CS101_2", ["24000003"])

    assert section1.code == "CS101_1"
    assert "24000001" in section1.students
    assert section1.student_count == 2
    assert section2.student_count == 1

    # 2. Test Exam
    exam = solver.Exam("CS101", 3, [section1, section2], [0, 1], [0])

    assert exam.code == "CS101"
    assert exam.credits == 3
    assert len(exam.sections) == 2
    assert exam.sections[0].code == "CS101_1"
    assert exam.feasible_slots == [0, 1]
    assert exam.feasible_rooms == [0]

    # Test các thuộc tính readonly mới
    assert exam.section_count == 2
    assert exam.student_count == 3  # (2 từ sec1 + 1 từ sec2)
...
def test_ant_colony_initialization(sample_data):
    ...
    colony = solver.AntColony(hp, exams, timestamps, rooms, base_seed=42)
...
def test_ant_colony_run_one_iteration(sample_data):
    ...
    colony = solver.AntColony(hp, exams, timestamps, rooms, base_seed=100)
...
def test_ant_colony_full_run_with_callback(sample_data):
    ...
    colony = solver.AntColony(hp, exams, timestamps, rooms, base_seed=999)
...
def test_solution_schedule_integrity(sample_data):
    ...
    colony = solver.AntColony(hp, exams, timestamps, rooms, base_seed=777)
...
def test_solver_determinism_with_same_seed(sample_data):
    ...
    colony_a = solver.AntColony(hp, exams, timestamps, rooms, base_seed=12345)

    colony_b = solver.AntColony(hp, exams, timestamps, rooms, base_seed=12345)
...
def test_solver_stochasticity_with_different_seeds(sample_data):
    ...
    colony_a = solver.AntColony(hp, exams, timestamps, rooms, base_seed=111)

    colony_b = solver.AntColony(hp, exams, timestamps, rooms, base_seed=222)
...
def test_edge_case_minimum_credits(sample_data):
    ...
    sec1 = solver.ExamSection("MIL001_1", ["24000001", "24000002"])
    sec2 = solver.ExamSection("PE1002_1", ["24000002", "24000003"])

    border_exams = [
        solver.Exam("MIL001", 1, [sec1], [0], [0]),
        solver.Exam("PE1002", 1, [sec2], [0], [0]),
    ]

    colony = solver.AntColony(hp, border_exams, timestamps, rooms, base_seed=42)
